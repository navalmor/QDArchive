...
# #         return " ".join(extract_text(item) if isinstance(item, dict) else str(item) for item in value if item is not None)
# #     return str(value)
...
# # def log_study_summary(logger: logging.Logger, study_id: Any, metadata: Dict[str, Any]) -> None:
# #     logger.info("Study loaded: source_project_id=%s title=%s", study_id, metadata.get("Title") or "<no-title>")
# #     logger.debug(
# #         "Study metadata: source_project_id=%s kindOfData=%s dataFormat=%s documents=%s license=%s",
# #         study_id,
...
# # def log_score_summary(logger: logging.Logger, source_project_id: Any, score_result: ScoreResult) -> None:
...
# #     logger.debug(
# #         "Score breakdown: source_project_id=%s kindOfData=%s dataFormat=%s documents=%s abstract=%s methodology=%s reasons=%s",
# #         source_project_id,
...
# #     strong_positive = ["qualitative", "transcript", "transcripts", "interview", "interviews", "semi structured interview", "conducted interview"]
...
# #     if access == "controlled":
# #         return [(None, None, STATUS_RESTRICTED)], None, [], None
...
# #     keywords: List[str],
# # ) -> None:
# #     logger.info("Processing study: source_project_id=%s", study_id)
...
# #         logger.error("Failed to fetch study detail: source_project_id=%s", study_id)
# #         return

# #     metadata = ((detail.get("data") or {}).get("getStudyItem"))
# #     if not metadata:
# #         summary["counts"]["failed_projects"] += 1
# #         logger.error("Study detail returned empty data: source_project_id=%s", study_id)
# #         return
...
# #         logger.info("Project classified into UKDS numeric bucket: source_project_id=%s dataFormat=%s", study_id, extract_text(metadata.get("DataFormat")))
...
# #         summary["counts"]["skipped_projects"] += 1
# #         logger.info("Project is not in allowed qualitative labels: source_project_id=%s label=%s", study_id, score_result["label"])
...
# #         summary["counts"]["skipped_projects"] += 1
# #         summary["counts"]["failed_projects"] += 1
# #         logger.error("Project staging failed: source_project_id=%s error=%s", study_id, hard_error)
# #         return
...
# #     logger.info("Project refresh completed: source_project_id=%s", study_id)
